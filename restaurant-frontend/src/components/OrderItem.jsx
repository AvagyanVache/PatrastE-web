...
export default function OrderItem({ order, onAccept, onDecline }) {
  ...
  return (
    <div className={`shadow-lg rounded-xl p-5 border-l-8 ${statusColor} hover:shadow-xl transition-shadow mb-4`}>
      {/* 1. Header: Always Visible (ID, Status, Restaurant Name, Total Price, Toggle) */}
      <div className="border-b pb-2">
        <div className="flex justify-between items-start mb-2">
          {/* Left Side: ID and Restaurant */}
          <div>
  ...
  ֏{order.totalPrice ? order.totalPrice.toFixed(2) : 'N/A'}
      ...
      {/* 2. Collapsible Content: Renders only when isExpanded is true */}
      {isExpanded && (
        <div className="pt-3">
          
          {/* Restaurant Address (Secondary Detail) */}
      ...
      {/* 3. Action Buttons: Always at the bottom, only show for pending orders */}
      {onAccept && onDecline && (
